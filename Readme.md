...
#### Безопасность:
- **JWT-авторизация через Depends**:
  - Автоматическая валидация токена для protected-эндпоинтов.
  - Интеграция: FastAPI's `Depends` с кастомным `get_current_user()`.
  - Защищённые методы: все POST/PUT/DELETE-операции.
...
| name            | VARCHAR(60)   | NOT NULL                    | Имя пользователя                            |
| email           | VARCHAR(320)  | UNIQUE, INDEX, NOT NULL     | Email пользователя (используется для входа) |
| hashed_password | VARCHAR(1024) | NOT NULL                    | Хешированный пароль                         |
| is_active       | BOOLEAN       | DEFAULT TRUE                | Активна ли учетная запись                   |
| role            | VARCHAR(60)   | DEFAULT 'user'              | Роль пользователя (права доступа)           |

### 8. Таблица: `refresh_tokens` (Refresh-токены)
...
5. **refresh_tokens → user**
   - Тип: Один-к-одному
   - Описание: Каждый токен принадлежит одному пользователю. При удалении пользователя все его токены автоматически удаляются (`ON DELETE CASCADE`)
